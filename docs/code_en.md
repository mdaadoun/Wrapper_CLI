...
- **`utils/`:** Cross-cutting utilities (cost calculator, caching).
  - **`cache.py` (`ContentCache`, `compute_content_hash`):** SHA-256 content hashing and disk-backed local JSON caching module (`~/.cache/ai_watcher/cache.json`). Manages cache retrieval, TTL validation, serialization of `AnalysisReport` with `is_cached=True`, atomic cache persistence, and purging.
